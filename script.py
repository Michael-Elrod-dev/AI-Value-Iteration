...
def value_iteration():
    ...
    all_value_functions = [np.copy(value_function)]  
    all_best_actions = [np.copy(best_actions)]  

    p = 0.7  # Primary action probability (Noise = 0.3)
            ...
            for col in range(len(grid[0])):
                if grid[row][col] not in ['X', 'T']:
                    ...
                    for action in actions:
                        ...
                        intended_value = rewards.get(grid[new_row][new_col], 0) + gamma * value_function[new_row][new_col] if grid[new_row][new_col] != 'X' else value_function[row][col]
                        
                        # Compute value considering other actions with (1-p)/3
                        remaining_value = 0
                        for alt_action in actions:
                            if alt_action != action:
                                alt_new_row, alt_new_col = row, col
                                if alt_action == 'up':
                                    alt_new_row = max(row - 1, 0)
                                elif alt_action == 'down':
                                    alt_new_row = min(row + 1, len(grid) - 1)
                                elif alt_action == 'left':
                                    alt_new_col = max(col - 1, 0)
                                elif alt_action == 'right':
                                    alt_new_col = min(col + 1, len(grid[0]) - 1)

                                alt_value = rewards.get(grid[alt_new_row][alt_new_col], 0) + gamma * value_function[alt_new_row][alt_new_col] if grid[alt_new_row][alt_new_col] != 'X' else value_function[row][col]
                                remaining_value += alt_value * ((1-p)/3)

                        actual_value = p * intended_value + remaining_value
